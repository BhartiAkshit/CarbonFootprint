...
class Favorite(models.Model):
    ...
    tip = models.ForeignKey(EmissionReductionTip, on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.user.username}'s Favorite - {self.tip_id}"
    

class EmissionGoal(models.Model):    
    owner = models.OneToOneField(User, on_delete=models.CASCADE, related_name='emission_goal') 
    petrol_goal = models.FloatField(null=True, blank=True) 
    diesel_goal = models.FloatField(null=True, blank=True) 
    electricity_goal = models.FloatField(null=True, blank=True) 
    food_goal = models.FloatField(null=True, blank=True) 
    total_goal = models.FloatField(null=True, blank=True) 

 

    def save(self, *args, **kwargs):     
        # Calculate and update the total goal before saving 
        self.total_goal = sum(goal for goal in [self.petrol_goal, self.diesel_goal, self.electricity_goal, self.food_goal] if goal is not None) 
        super(EmissionGoal, self).save(*args, **kwargs) 

 

    def __str__(self): 
        return f"Emission Goal for {self.owner.username}" 