...
#     # Check if provided credentials match the predefined admin login
#     if email == ADMIN_LOGIN_CREDENTIALS['email'] and password == ADMIN_LOGIN_CREDENTIALS['password']:
#         # Assuming you have an AdminUser model with is_staff=True
#         admin_user = get_user_model().objects.filter(email=email, is_staff=True).first()
